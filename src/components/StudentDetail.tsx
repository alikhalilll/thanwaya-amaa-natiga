...
export default function StudentDetail({
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6">
      <button
        onClick={onBack}
        className="mb-4 inline-flex items-center gap-1 rounded-lg bg-white/5 px-3 py-2 text-xs text-white/70 hover:bg-white/10 transition"
      >
...
function StudentBody({
  ...
  return (
    <motion.div
      ...
      className="space-y-4"
    >
      <div className={`glass rounded-3xl p-6 ring-1 ${tier.ring}`}>
        <div className="flex flex-wrap items-center gap-2">
          <span className={`rounded-full border px-3 py-1 text-[11px] ${status.chip}`}>
            {statusLabel || status.label}
          </span>
          {tierName && (
            <span className={`rounded-full border px-3 py-1 text-[11px] ${tier.chip}`}>
              التقدير: {tierName}
            </span>
          )}
        ...
        <h1 className="mt-3 text-2xl sm:text-3xl md:text-4xl font-black text-white break-words">
          {record.name}
        </h1>

        <div className="mt-5 flex flex-col items-center gap-5 sm:flex-row sm:justify-between">
          <div
            className={`w-full sm:w-auto rounded-3xl bg-gradient-to-br ${degreeColor(
              record.degree,
              degreeMax,
            )} px-6 py-5 text-white shadow-2xl text-center`}
          >
          ...
          <div className="grid grid-cols-3 gap-3 w-full sm:w-auto">
            <MetricCard label="الترتيب العام" value={`#${toArabicDigits(record.rank.toLocaleString('en-US'))}`} />
            <MetricCard label="داخل الحالة" value={`#${toArabicDigits(record.rankInStatus.toLocaleString('en-US'))}`} />
            <MetricCard label="أعلى من" value={`${toArabicDigits(percentile.toFixed(1))}٪`} />
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          <button
            ...
            className="rounded-lg bg-white/5 px-3 py-2 text-xs text-white/80 hover:bg-white/10 transition"
          >
            نسخ رابط النتيجة
          </button>
          <button
            ...
            className="rounded-lg bg-brand-500/20 border border-brand-400/40 px-3 py-2 text-xs text-brand-100 hover:bg-brand-500/30 transition"
          >
            مشاركة
          </button>
          <button
          ...
          <div className="mt-2 grid gap-2">
            {similar.map((s) => (
              <a
                key={s.seat}
                href={`#/s/${s.seat}`}
                className="flex items-center justify-between gap-3 rounded-xl border border-white/5 bg-white/5 p-3 hover:bg-white/10 transition"
              >
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm text-white">{s.name}</div>
                  <div className="text-[11px] text-white/50">
                    رقم الجلوس: {toArabicDigits(s.seat)} · #{toArabicDigits(s.rank.toLocaleString('en-US'))}
                  </div>
                </div>
                <div className="rounded-lg bg-white/10 px-3 py-1 text-sm font-bold text-white">
...
function MetricCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="glass rounded-xl px-3 py-2 text-center">
      ...
      <div className="mt-0.5 text-sm sm:text-base font-bold text-white">{value}</div>
    </div>
  );
}
